...
def __union_with_existing_data(staging_data, args, glueContext):
    ...
    existing_data = glueContext.create_dynamic_frame_from_options(
        connection_type = "s3",
        format = "parquet",
        connection_options = {"paths": [target_path]})

    existing_count = existing_data.toDF().count()
    log.error(f'Existing rows found: {existing_count}')
    if existing_data.toDF().take(1):
        log.error('merging with existing data')
        return existing_data.toDF() \
            .union(staging_data.toDF())
    else:
        return staging_data.toDF()
...
def __repartition_and_store(result_df, args, glueContext):
    target_path = args["target_path"]
    repartitioned_stream = DynamicFrame.fromDF(result_df, glueContext, 'result_df') \
        .repartition(2)
